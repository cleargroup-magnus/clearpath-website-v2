...
/* ─────────────────────────── GTM Engineering ────────────────────────── */
/* Clay/Apollo/Zapier/HubSpot U-shape pipeline                            */
export function GTMAnim({ active }: { active: boolean }) {
  const phase = usePhase(active, 2200, 1800, 2600);

  /* Final pipeline positions */
  const tools = [
    { id: "Apollo",  cx: 22,  cy: 28,  label: "Apollo",  dx: +34, dy: +7  },
    { id: "Clay",    cx: 138, cy: 28,  label: "Clay",    dx: -34, dy: +7  },
    { id: "Zapier",  cx: 138, cy: 62,  label: "Zapier",  dx: -34, dy: -5  },
    { id: "HubSpot", cx: 22,  cy: 62,  label: "HubSpot", dx: +34, dy: -5  },
  ];

  /* Connection paths for U-shape */
  const paths = [
    { d: "M 41 28 L 119 28",  dash: 80,  arrowPoints: "119,28 114,25 114,31" },
    { d: "M 138 37 L 138 53", dash: 18,  arrowPoints: "138,53 135,48 141,48" },
    { d: "M 119 62 L 41 62",  dash: 80,  arrowPoints: "41,62 46,59 46,65"    },
  ];

  const redDot = "oklch(0.55 0.22 25)";

  return (
    <div style={{ width: "100%", maxWidth: 172, display: "flex", flexDirection: "column", gap: 10 }}>
      <svg viewBox="0 0 160 92" style={{ width: "100%", overflow: "visible" }}>

        {/* Connection paths — phase >= 1 */}
        {paths.map((p, i) => (
          <path key={i} d={p.d}
            fill="none"
            stroke={phase === 2 ? G : B}
            ...
            strokeDashoffset={phase >= 1 ? 0 : p.dash}
            strokeOpacity={phase >= 1 ? 0.55 : 0}
            style={{ transition: `stroke-dashoffset .55s ease ${i * .15}s, stroke-opacity .4s ease ${i * .15}s, stroke .4s ease` }}
          />
        ))}

        {/* Arrow heads */}
        {paths.map((p, i) => (
          <polygon key={i} points={p.arrowPoints}
            fill={phase === 2 ? G : B}
            opacity={phase >= 1 ? 1 : 0}
            style={{ transition: `opacity .4s ease ${i * .15 + .3}s, fill .4s ease` }}
          />
        ))}

        {/* Travelling data dots — phase 1 */}
        {phase === 1 && paths.map((p, i) => (
          <circle key={i} r="2.5" fill={BL}>
            <animateMotion dur="1.2s" begin={`${i * 0.4}s`} repeatCount="indefinite" path={p.d} />
        ...
        {/* Tool chips */}
        {tools.map((t, i) => {
          /* In phase 0 chips cluster in center; phase >= 1 they snap to corners */
          const tx = phase === 0 ? t.dx : 0;
          const ty = phase === 0 ? t.dy : 0;
          const statusColor = phase === 0 ? redDot : phase === 1 ? B : G;

          return (
            <g key={t.id}
              style={{
                transform: `translate(${tx}px, ${ty}px)`,
                transition: `transform .55s cubic-bezier(0.34, 1.56, 0.64, 1) ${i * .08}s`,
              }}>
              {/* Chip body */}
              <rect
                x={t.cx - 18} y={t.cy - 9}
                width={36} height={18} rx={5}
                fill={phase === 2 ? `${G}14` : `${B}14`}
                stroke={phase === 2 ? G : phase === 1 ? B : "rgba(255,255,255,0.18)"}
                strokeWidth="1"
                strokeOpacity={phase === 0 ? 0.3 : 0.6}
              ...
              {/* Status dot */}
              <circle
                cx={t.cx + 16} cy={t.cy - 8} r={3}
                fill={statusColor}
                style={{ transition: `fill .4s ease ${i * .08}s` }}
              />
            </g>
          );
        })}

        {/* Phase 0 — not connected label */}
        <g style={{ opacity: phase === 0 ? 1 : 0, transition: "opacity .4s ease" }}>
          <rect x="36" y="72" width="88" height="14" rx="4"
            fill="oklch(0.55 0.22 25 / 0.12)"
            stroke="oklch(0.55 0.22 25 / 0.35)"
            strokeWidth=".7"
          />
          <text x="80" y="81.5" textAnchor="middle" fontSize="6"
            fill="oklch(0.70 0.18 25)"
            fontFamily="Inter, sans-serif" fontWeight="600">
            Tools not connected
          </text>
        </g>
        ...
        {/* Phase 2 — metric row */}
        <g style={{ opacity: phase === 2 ? 1 : 0, transition: "opacity .45s ease .25s" }}>
          {/* Live pulse dot */}
          <circle cx="15" cy="79" r="3.5" fill={G} style={{ animation: phase === 2 ? "live-pulse 1.2s ease infinite" : "none" }} />
          {/* Metric background */}
          <rect x="22" y="73" width="124" height="13" rx="4"
            fill={`${G}10`} stroke={G} strokeWidth=".7" strokeOpacity=".45"
          />
          <text x="84" y="81.5" textAnchor="middle" fontSize="6.5"
            fill={GL} fontFamily="Inter, sans-serif" fontWeight="700">
            23 leads/day · Fully automated
          </text>
        </g>
      </svg>

      <PhaseBar phase={phase} labels={["Disconnected", "Wiring up", "Pipeline live"]} />
    </div>
  );
}
  ...
  return (
    <div style={{ width: "100%", maxWidth: 172, display: "flex", flexDirection: "column", gap: 9 }}>
      ...
      <div style={{ opacity: phase === 2 ? 1 : 0, transition: "opacity .4s ease .1s" }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {[{ l: "ROAS", v: "4.2x" }, { l: "Clicks", v: "+38%" }, { l: "CPA", v: "-22%" }].map(({ l, v }) => (
            <div key={l} style={{
